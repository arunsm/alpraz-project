...
brainStates = 'betas'; % possible values = {'betas', 'spinTest'}
normalizationMethod = 'EuclideanNorm'; % possible values = {'EuclideanNorm', 'none'}
...
nIterations_nullModel2 = 1; % number of iterations for structural null models

%% iterating through parameter choices
for iter_nullModel2 = 1:nIterations_nullModel2
    switch structuralEdges
    ...
    resultsDir = strcat('/data/joy/BBL/studies/alpraz/scripts/ControlCode/Results/', ...
        date, '_avge_FD_thresh_', num2str(avge_FD_thresh), ...
        '_parcelCoverageThresh_', num2str(parcelCoverageThresh), ...
        '_', normalizationMethod, '_', controlSet, '_', ...
        structuralEdges, '_', brainStates, '/');
    ...
    fprintf(fid, 'controlAlgorithm,normalizationMethod,avge_FD_thresh,parcelCoverageThresh,T,controlInputs,structuralEdges,brainStates\n%s,%s,%d,%d,%d,%s,%s,%s', ...
        controlAlgorithm, normalizationMethod, avge_FD_thresh, parcelCoverageThresh, T, controlSet, structuralEdges, brainStates);
    fclose(fid);
        ...
        for i = 1:nSubjects
            ...
            for contrast_idx = 1:nContrasts
                ...
                for j = 1:numel(allSessionIDs_currentSubject)
                    ...
                    if current_avge_FD > avge_FD_thresh
                        ...
                        if isstruct(X)
                            ...
                            switch brainStates
                                case 'betas'
                                    betas = x(parcelsToInclude); % using beta values from first-level GLM as brain states
                                case 'spinTest'
                                    scrambled_betas_savePath = strcat('spinTestResults', filesep, currentSubjectID_str, '_', currentSessionID_str, '_', currentTaskID, '_', currentContrastLabel, '_', atlasLabel);
                                    betas = calculateScrambledBetas(x, scrambled_betas_savePath, nIterations_spinTest); % using scrambled activation patterns calculated using spin-test
                            end
                            
                            switch normalizationMethod
                                case 'EuclideanNorm'
                                    xf = betas/norm(betas); % normalizing brain states by Euclidean norm
                                case 'none'
                                    xf = betas; % no normalization applied
                            end
                        else
                        ...
                        % if implementing spin test, xf will be a [nNodes x nPermutations] matrix
                        if size(xf, 2) > 1
                            for iter_spinTest = 1:nIterations_spinTest
                                ...
                                switch controlAlgorithm
                                    case 'optimalControl'
                                        ...
                                        [stateTrajectories_persistence, controlInputs_persistence, numericalError] = opt_eng_cont(A_slab, T, B_slab, current_xf, current_xf, rho, S, true);
                                    ...
                                    case 'minimalControl'
                                        % calculating persistence of brain state - control energy needed
                                        % to maintain state xf
                                        [stateTrajectories_persistence, controlInputs_persistence, numericalError] = min_eng_cont(A_slab, T, B_slab, current_xf, current_xf, true);
                                        stateTrajectories_persistence = stateTrajectories_persistence(:, 1:nNodes_currentSubject);
                                        energyCost_persistence = trapz(controlInputs_persistence.^2)/nTimeSteps; % calculating the total energetic cost as the sum of integral of squared energy trajectories, divided by number of time steps
                                end
                                
                                persistence_allNodes(iter_spinTest) = sum(energyCost_persistence);
                                persistence_visual(iter_spinTest) = sum(energyCost_persistence(finalLabels_slab==1));
                                persistence_ventralAttention(iter_spinTest) = sum(energyCost_persistence(finalLabels_slab==4));
                                persistence_limbic(iter_spinTest) = sum(energyCost_persistence(finalLabels_slab==5));
                                persistence_frontoparietal(iter_spinTest) = sum(energyCost_persistence(finalLabels_slab==6));
                                persistence_DMN(iter_spinTest) = sum(energyCost_persistence(finalLabels_slab==7));
                                persistence_subcortex(iter_spinTest) = sum(energyCost_persistence(finalLabels_slab==8));
                                
                                controlImpact_currentSubject = zeros(1, nNodes_currentSubject); % setting control impact to zeros
                            end
                        else
                        ...
                        switch currentTaskID
                            case 'task-emotionid'
                                allControlEnergies_emotionid(ctr, :) = {currentSubjectID, currentSessionID, currentContrastLabel, ...
                                    ...
                                    current_STAI_TRAIT, current_alpraz_levels, current_avge_FD};
                                allControlTrajectories_emotionid(ctr, :) = {currentSubjectID, currentSessionID, currentContrastLabel, ...
                                    parcelsToInclude_idx, betas, xf, controlInputs_persistence, stateTrajectories_persistence, controlImpact_currentSubject, ...
    ...
    allControlEnergies_emotionid = cell2table(allControlEnergies_emotionid, 'VariableNames', {'subjectID', 'sessionID', 'contrast', ...
        ...
        'rtmdn_threatcorr', 'rtmdn_nonthreatcorr', 'rtmdn_neutralcorr', 'STAI_TRAIT', 'alpraz_levels', 'avge_FD'});
    ...
    allControlTrajectories_emotionid = cell2table(allControlTrajectories_emotionid, 'VariableNames', {'subjectID', 'sessionID', 'contrast', ...
        'parcelsToInclude_idx', 'betas', 'xf', 'controlInputs_persistence', 'stateTrajectories_persistence', 'controlImpact_persistence', 'numericalError', ...
...
%     writetable(allControlEnergies_emotionrec, strcat(resultsDir, 'allControlEnergies_emotionrec_iteration', num2str(iter_nullModel2), '.csv'));
    ...
    baseName_emotionid = strcat(resultsDir, 'allControlEnergies_emotionid');
    baseName_emotionrec = strcat(resultsDir, 'allControlEnergies_emotionrec');
