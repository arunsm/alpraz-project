function [] = printSupplementaryDataFiles9(allControlEnergies_emotionid, allControlEnergies_emotionrec)
parameters
nIterations = 500;

resultsDirCurrentFigure = strcat(resultsDir, filesep, 'SupplementaryDataFiles9', filesep);
if ~exist(resultsDirCurrentFigure)
    mkdir(resultsDirCurrentFigure)
end

% compare test statistics from mixed model against spin test null models
...
for i = 1:nContrasts
    ...
    for j = 1:nIterations
        ...
        currentModelFormula = strcat('persistence_allNodes_', num2str(j), ' ~ drug + group + drug*group + avge_FD + gender + age + (1|subjectID)');
    ...
    ylabel('Coefficients');
    saveas(f, strcat(resultsDirCurrentFigure, 'emotionid_', currentContrast, '_spatialNullCoefficients.eps'));
    close(f);
end
...
% load results of null model
resultsDir_nullModel = 'Results/minimalControl/avge_FD_thresh_0.5_parcelCoverageThresh_0.5_EuclideanNorm_allNodes_QA_spinTest/';
allControlEnergies_emotionrec_nullModel = readtable(strcat(resultsDir_nullModel, 'allControlEnergies_emotionrec.csv'));
...
for i = 1:nContrasts
    ...
    saveas(f, strcat(resultsDirCurrentFigure, 'emotionrec_', currentContrast, '_spatialNullCoefficients.eps'));
end
end