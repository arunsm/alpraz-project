...
for i = 1:nContrasts
    ...
    mixedModel_emotionid_currentContrast = fitlme(allControlEnergies_emotionid_currentContrast, 'persistence_allNodes ~ alpraz_levels + group + alpraz_levels*group + gender + age + (1|subjectID)', 'FitMethod', 'ML');
...
for i = 1:nContrasts
    ...
    %h = kstest(zscore(persistence_allNodes));
    %figure, histogram(persistence_allNodes); title(strcat('KS test: ', num2str(h)));
    mixedModel_emotionrec_currentContrast = fitlme(allControlEnergies_emotionrec_currentContrast, 'persistence_allNodes ~ alpraz_levels + group + alpraz_levels*group + gender + age + (1|subjectID)', 'FitMethod', 'ML');
