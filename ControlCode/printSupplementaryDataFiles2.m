function [] = printSupplementaryDataFiles2(allControlEnergies_emotionid, allControlEnergies_emotionrec)

parameters
resultsDirCurrentFigure = strcat(resultsDir, filesep, 'SupplementaryDataFiles2', filesep);
...
for i = 1:nContrasts
    ...
    mixedModel_emotionid_currentContrast = fitlme(allControlEnergies_emotionid_currentContrast, 'persistence_allNodes ~ alpraz_levels + group + alpraz_levels*group + avge_FD + gender + age + (1|subjectID)', 'FitMethod', 'ML');
    save(strcat(resultsDirCurrentFigure, 'mixedModel_emotionid_alpraz_levels_', currentContrast, '.mat'), 'mixedModel_emotionid_currentContrast');
    printFileName = strcat(resultsDirCurrentFigure, 'mixedModel_emotionid_alpraz_levels_', currentContrast, '.xlsx');
...
for i = 1:nContrasts
    ...
    %h = kstest(zscore(persistence_allNodes));
    %figure, histogram(persistence_allNodes); title(strcat('KS test: ', num2str(h)));
    mixedModel_emotionrec_currentContrast = fitlme(allControlEnergies_emotionrec_currentContrast, 'persistence_allNodes ~ alpraz_levels + group + alpraz_levels*group + avge_FD + gender + age + (1|subjectID)', 'FitMethod', 'ML');
    save(strcat(resultsDirCurrentFigure, 'mixedModel_emotionrec_alpraz_levels_', currentContrast, '.mat'), 'mixedModel_emotionrec_currentContrast');
    printFileName = strcat(resultsDirCurrentFigure, 'mixedModel_emotionrec_alpraz_levels_', currentContrast, '.xlsx');
